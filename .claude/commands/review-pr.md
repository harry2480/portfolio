...
引数を解析して **レビューモード** を決定する：

- 引数に `diff` が含まれる場合 → **差分モード**（diff のみを見てレビュー）
- `diff` が含まれない場合 → **フルモード**（プロジェクト全体のコンテキストでレビュー）

PR番号またはURLは `diff` を除いた残りの引数から取得する。
...
変更ファイル一覧（`files` フィールド）をもとに、関連するファイルをプロジェクトから読み込んでコンテキストを把握する：

- 変更ファイルそのものを Read で読む
- 変更ファイルが import / require しているモジュールや関連コンポーネントがあれば追加で読む
- テスト対象のユニットテストファイルがあれば読む
- CLAUDE.md / README.md など設計方針ドキュメントを参照する

**差分モードの場合は上記のファイル読み込みは行わない。diff の内容だけでレビューする。**
